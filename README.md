...
- Nipun-AI manifests under `deploy/k8s/` with Argo app `deploy/argocd/application.yaml`
- TradingAgents manifests under `deploy/tradingagents/k8s/` with Argo app `deploy/argocd/tradingagents-application.yaml`
...
3. Access Nipun-AI via static IP `10.0.0.220`:
- Frontend: `https://10.0.0.220`
- Worker API: `https://10.0.0.220/analyze` and `https://10.0.0.220/health`

4. Access TradingAgents terminal via static IP `10.0.0.219`:
- HTTPS terminal: `https://10.0.0.219`
...
Argo tracks this fork (`https://github.com/bpeet1/Nipun-AI`) and Renovate (`renovate.json` + `.github/workflows/renovate.yml`) handles automated dependency/runtime image updates for both deployment trees.
